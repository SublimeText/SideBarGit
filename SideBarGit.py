...
class SideBarGit:
    ...
    def run2(
        ...
        if not refresh_funct_view:
            ...
            object.item = SideBarItem(
                refresh_funct_item, os.path.isdir(refresh_funct_item)
            )
        ...
        try:
            ...
            if (
                stdout.find("fatal:") == 0
                or stdout.find("error:") == 0
                or stdout.find("Permission denied") == 0
                or stderr
            ):
                if debug:
                    print("FAILED")
        ...
        if failed:
            try:
                ...
                if not stdout and not stderr:
                    return False
                if (
                    stdout.find("Permission denied") == 0
                    or stdout.find("fatal: The remote end hung up unexpectedly") == 0
                ):
                    self.alert(
                        (stdout or "")
                        + "\n"
            ...
            if (
                stdout != ""
                and refresh_funct_view == False
                and (
                    object.to_status_bar
                    or " ".join(object.command).find("git push") == 0
                    or stdout.find("nothing to commit") == 0
                )
            ):
                self.status(stdout)
            else:
                ...
                if refresh_funct_view == False:
                    ...
                    try:
                        view.settings().set(
                            "SideBarGitToStatusBar", object.to_status_bar
                        )
                    except:
    ...
    def confirm(self, message, function, arg1):
        if int(sublime.version()) >= 2186:
            ...
            sublime.active_window().show_input_panel(
                ...
                message,
                functools.partial(function, arg1, True),
                None,
                None,
            )
    ...
    def prompt(self, message, default, function, arg1):
        ...
        sublime.active_window().run_command("hide_panel")
        sublime.active_window().show_input_panel(
            message, default, functools.partial(function, arg1, True), None, None
        )
    ...
    def quickPanel(self, function, extra, data):
        ...
        window.show_quick_panel(
            data, functools.partial(self.quickPanelDone, function, extra, data)
        )
        ...
        for item in items:
            ...
            if os.path.exists(item.join(".git")):
                try:
                    ...
                    repos[index].items = []
                repos[index].items.append(
                    SideBarItem(original, os.path.isdir(original))
                )
        return repos
