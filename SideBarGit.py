...
from SideBarGit.SideBarAPI import SideBarItem
    ...
    def run2(
        ...
        refresh_funct_syntax_file=False):
            ...
            if failed:
                ...
                if stdout != '' and refresh_funct_view is False and (object.to_status_bar or " ".join(object.command).find('git push') == 0 or stdout.find('nothing to commit') == 0):
                    self.status(stdout)
                else:
                    if stdout == '' and refresh_funct_view is False:
                        try:
                            self.status(object.no_results)
                        except:
                            self.status('No output to show')
                        return True
                    if stdout == '' and refresh_funct_view is not False:
                        try:
                    ...
                    if refresh_funct_view is False:
