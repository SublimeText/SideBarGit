...
class SideBarSelection:
    ...
    def _obtainSelectionInformationBasic(self):
        if not self._obtained_selection_information_basic:
            ...
            for path in self._paths:
                if self._has_directories is False and os.path.isdir(path):
                    self._has_directories = True
                if self._has_files is False and os.path.isdir(path) is False:
    ...
    def isNone(self, path):
        if path is None or path == '' or path == '.' or path == '..' or path == './' or path == '../' or path == '/' or path == '//' or path == '\\' or path == '\\\\' or path == '\\\\\\\\' or path == '\\\\?\\' or path == '\\\\?' or path == '\\\\\\\\?\\\\':
            return True
        else:
            return False
...
class SideBarItem:
    ...
    def copy(self, location, replace=False):
        ...
        if location.exists() and replace is False:
            return False
